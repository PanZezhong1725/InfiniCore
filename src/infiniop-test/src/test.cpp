...
std::unordered_map<std::string, const TestBuilder> TEST_BULDERS = TEST_BUILDER_MAPPINGS;
...
std::shared_ptr<Result> runTest(const GGUFFileReader &gguf_reader,
    ...
    if (name_meta != meta.end()) {
        ...
        auto builder = TEST_BULDERS.find(op_name)->second;
