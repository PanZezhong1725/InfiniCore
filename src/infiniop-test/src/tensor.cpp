...
inline void caculateTensorMemory(size_t &size, size_t &offset,
...
Memory::Memory(std::shared_ptr<FileMapping> file_mapping, void *ptr, size_t size) {
...
Tensor::Tensor(const GGUFTensorInfo *info,
               std::shared_ptr<FileMapping> gguf_file_mapping,
               const void *ggml_ptr,
    ...
    std::vector<ptrdiff_t> contigous_strides(ndim);
    for (size_t i = 0; i < ndim; i++) {
        _shape[i] = static_cast<size_t>(info->shape[ndim - 1 - i]);
        if (i == 0) {
            contigous_strides[ndim - 1] = (ptrdiff_t)1;
        } else {
            contigous_strides[ndim - 1 - i] = (ptrdiff_t)_shape[ndim - i] * contigous_strides[ndim - i];
        }
    }

    if (strides_meta == nullptr) {
        for (size_t i = 0; i < ndim; i++) {
            _strides[i] = contigous_strides[i];
        }
    } else {
        for (size_t i = 0; i < ndim; i++) {
            _shape[i] = static_cast<size_t>(info->shape[ndim - 1 - i]);
    ...
    size_t size;
    caculateTensorMemory(size, _offset, _shape, _strides, ggmlTypeSize(_ggml_type));
...
Tensor::Tensor(std::shared_ptr<Memory> memory, size_t offset,
               ...
               GGML_TYPE dtype) : _shape(shape), _strides(strides), _ggml_type(dtype), _memory(memory), _offset(offset) {
